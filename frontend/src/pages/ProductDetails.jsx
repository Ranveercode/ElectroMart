  ...
  useEffect(() => {
    const fetchProductData = async () => {
      try {
        ...
        const productRes = await fetch(`https://electro-mart-qalg.vercel.app/api/products/${id}`);
        if (!productRes.ok) throw new Error("Product not found");
        const productData = await productRes.json();
        setProduct(productData);

        // Fetch related products in same segment
        const relatedRes = await fetch(`https://electro-mart-qalg.vercel.app/api/products?segment=${productData.segment}`);
        if (relatedRes.ok) {
